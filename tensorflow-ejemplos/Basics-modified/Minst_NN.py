...
# Pooling is like downsampling
def max_pool_2x2(x):
  return tf.nn.max_pool(x, ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1], padding='SAME')


#First Convolutional Layer (los pesos son convoluciones, que e aplicaran con conv2 a una imagen)
...
# MaxPool (downsampling) 2x2 -> Size/4
h_pool1 = max_pool_2x2(h_conv1)


# Second Convolutional Layer
# The second layer will have 64 features for each 5x5 patch. (and now has 32 input channels, the features of each patch)
...
cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y_conv), reduction_indices=[1])) #error
train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy) #trainer
...
for i in range(10000):
  ...
  train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5}) #trainer
